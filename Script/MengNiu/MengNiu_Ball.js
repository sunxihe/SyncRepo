...
async function main() {
    for (const item of MengNiu) {
        ...
        for (const task of taskList.task) {
            ...
            if (task.refreshCycles[0].curRefreshValue < task.refreshCycles[0].maxRefreshTimes && task.taskId == "T003") {
                for (let i = task.refreshCycles[0].curRefreshValue; i < task.refreshCycles[0].maxRefreshTimes; i++) {
                    let decUserItem = await commonPost('/item/user/v1/decUserItem',{"sActiveId":activeid,"sUserId":id,"sItemId":"d004","iCount":10,"params":"","negative":false})
                    console.log(decUserItem)
                    let finishTask = await commonPost('/task/user/v1/finishTask',{"taskId":task.taskId,"sActiveId":activeid,"roleId":id})
        ...
        console.log("————————————")
        console.log("开始游戏")
        login.serverTime = Date.now()
        let logincheck = await commonPost("/game/game/local/logincheck",{"activeid":activeid,"info":login})
        for (let i = 0; i < logincheck.role.gameChance; i++) {
            let startgame = await commonPost('/game/game/local/startgame',{"activeId":activeid,"gameId":logincheck.role.gameId})
            console.log(startgame.singleGameLock)
            let endGameByMengniu = await commonPost('/game/game/local/endGameByMengniu',{"activeId":activeid,"gameId":logincheck.role.gameId,"singleGameLock":startgame.singleGameLock,"score":14.547999999998833})
