...
async function main() {
    for (const item of YongXin) {
        ...
        //签到
        console.log("开始签到")
        let sign = await commonPost("/signin",{"userId":userId})
        console.log(sign.ret_msg)
        console.log("————————————")
        //做任务
        let taskList = await commonPost("/task/list",{"userId":userId})
        for (const task of taskList.ret_module.specialTask) {
            console.log(`特殊任务：${task.name} 积分：${task.points}`)
            if (task.finishCount < task.totalCount) {
                for (let i = task.finishCount; i < task.totalCount; i++) {
                    let finish = await commonPost("/task/finish",{"userId":userId,"taskId":task.id})
                    console.log(finish.ret_msg)
                }
            } else {
                console.log("任务已完成")
            }
        }
        for (const task of taskList.ret_module.dailyTask) {
            console.log(`日常任务：${task.name} 积分：${task.points}`)
            let finish = await commonPost("/task/finish",{"userId":userId,"taskId":task.id})
            console.log(finish.ret_msg)
        }
        console.log("————————————")
...
async function commonPost(url,body = {}) {
    return new Promise(resolve => {
        const options = {
            url: `https://api.nblinks.cn/activityApi/points/v1${url}`,
            headers: {
                'content-length': '20',
                'xweb_xhr': '1',
                ...
                'referer': 'https://servicewechat.com/wx07cb93d4b2273a49/115/page-frame.html',
