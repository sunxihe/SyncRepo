...
async function main() {
    for (const item of MengNiu) {
        ...
        console.log("————————————")
        console.log("画龙领福气")
        let login = await commonPost("Login",`UID=${uid}`)
        console.log(login)
        for (let i = 0; i < login.result.GameCount; i++) {
            let GameRecord = await commonPost("GameRecord",`UID=${uid}&Difficulty=${login.result.Difficulty + 1}&IsSuccess=1&ActivityTimeID=106`)
            console.log(GameRecord)
            let Luckdraw = await commonPost("Luckdraw",`UID=${uid}&ActivityTimeID=106`)
